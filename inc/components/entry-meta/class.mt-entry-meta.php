 ...
 *
 */
if(!function_exists('Fargo_CallEntryMetaClass')) {
    /**
     *
     */
    function Fargo_CallEntryMetaClass()
    {

        // instantiate the class & load everything else
        Fargo_Entry_Meta_Output::getInstance();

    }
    add_action('wp_loaded', 'Fargo_CallEntryMetaClass');
}



if( !class_exists( 'Fargo_Entry_Meta_Output' ) ) {

    class Fargo_Entry_Meta_Output
    {
        ...
        protected function __construct() {
            add_action( 'fargo_single_entry_meta', array( $this, 'single_entry_meta_output' ), 1 );
            add_action( 'fargo_archive_meta_content', array( $this, 'archive_entry_meta_output' ), 1 );
            add_action( 'fargo_single_after_content', array( $this, 'single_content_tags' ), 1 );
        }
        ...
        public function single_entry_meta_output() {
            ...
            $categories_list = get_the_category_list( esc_html__( ', ', 'fargo' ) );
            $number_comments = get_comments_number();

            $display_post_posted_on_meta = get_theme_mod( 'fargo_enable_post_posted_on_blog_posts', 1 );
            $display_number_comments = get_theme_mod( 'fargo_enable_post_comments_blog_posts', 1 );
            ...
            if( $display_post_posted_on_meta == 1 ) {
                $output = '';
                 ...
                 $output .= '<div class="blog-post-meta">';
                    ...
                    $output .= ( ( $display_number_comments == 1 ) ? ( comments_open() ) ? ( $number_comments == 0 ) ? sprintf( '<span class="post-meta-comments"><i class="fa fa-comment-o"></i>'. __( 'No comments', 'fargo' ) .'</span>' ) : ( $number_comments > 1 ) ? sprintf( '<span class="post-meta-comments"><i class="fa fa-comment-o"></i><a class="meta-comments" href="%s" title="%s '. __( 'comments', 'fargo' ) .'">%s '. __( 'comments', 'fargo' ) .'</a></span>', get_comments_link(), $number_comments, $number_comments ) : sprintf( '<span class="post-meta-comments"><i class="fa fa-comment-o"></i><a class="meta-comments" href="%s" title="'. __( '1 comment', 'fargo' ) .'">'. __( '1 comment', 'fargo' ) .'</a></span>', get_comments_link() ) : sprintf( '<span class="post-meta-comments"><i class="fa fa-comment-o"></i>'. __( 'Comments are off for this post', 'fargo' ) .'</span>' ) : '' );
        ...
        public function archive_entry_meta_output() {
            ...
            $number_comments = get_comments_number();
            $categories_list = get_the_category_list( esc_html__( ', ', 'fargo' ) );
            $post_standard_enable_author = get_theme_mod( 'fargo_post_standard_enable_author', 1 );
        ...
        public function single_content_tags() {
            $display_tags_post_meta  = get_theme_mod( 'fargo_enable_post_tags_blog_posts', 1 );
