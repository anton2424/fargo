...
/**
 * Class Fargo_Related_Posts_Output
 *
...
if( !function_exists( 'Fargo_CallRelatedPostsClass' ) ) {
    /**
     *
     ...
     *
     */
    function Fargo_CallRelatedPostsClass()
    {
        $display_related_blog_posts = get_theme_mod('fargo_enable_related_blog_posts', 1);

        if ($display_related_blog_posts == 1) {

            // instantiate the class & load everything else
            Fargo_Related_Posts_Output::getInstance();
        }
    }
    add_action( 'wp_loaded', 'Fargo_CallRelatedPostsClass');
}


if( !class_exists( 'Fargo_Related_Posts_Output' ) ) {
	...
	class Fargo_Related_Posts_Output {
		...
		protected function __construct() {
			add_action( 'fargo_single_after_content', array( $this, 'output_related_posts' ), 3 );
		}
					...
					$output .= '<div class="row">';
						$output .= '<div class="col-sm-12">';
							$output .= '<div class="related-article-title">';
								$output .= __( 'Related Articles', 'fargo' );
						...
						while( $post_query->have_posts() ) {
							...
							$post_thumbnail = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'fargo-blog-post-related-articles' );
