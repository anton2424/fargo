...
if ( ! class_exists( 'MT_Notify_System' ) ) {
	...
	class MT_Notify_System {
		...
		public static function version_check( $ver ) {
			...
			return version_compare( $fargo['Version'], $ver, '>=' );
		}
		 ...
		 * @return bool
		 */
		public static function fargo_has_posts() {
		...
		public static function widget_importer_exporter_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: Widget Importer Exporter Plugin', 'fargo' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Activate: Widget Importer Exporter Plugin', 'fargo' );
			}
		...
		public static function wordpress_importer_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: WordPress Importer', 'fargo' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Activate: WordPress Importer', 'fargo' );
			}
		...
		public static function wordpress_importer_description() {
			$installed = self::check_plugin_is_installed( 'wordpress-importer' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress Importer to create the demo content.', 'fargo' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Importer to create the demo content.', 'fargo' );
			}
		...
		public static function widget_importer_exporter_description() {
			$installed = self::check_plugin_is_installed( 'widget-importer-exporter' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress widget importer to create the demo content', 'fargo' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Widget Importer to create the demo content.', 'fargo' );
			}
		...
		public static function create_plugin_title( $plugin_title, $plugin_slug ){
			$installed = self::check_plugin_is_installed( $plugin_slug );
			if ( ! $installed ) {
				return __( 'Install : ', 'fargo' ).$plugin_title;
			}elseif ( ! self::check_plugin_is_active( $plugin_slug ) && $installed ) {
				return __( 'Activate : ', 'fargo' ).$plugin_title;
			}else{
				return __( 'Update : ', 'fargo' ).$plugin_title;
			}
		}
